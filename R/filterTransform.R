...
filterTransform.lacen <- function(lacenObject,
  ...
  select_by_variance <- function(count_data, top_var, lnc_keep) {
    mads <- apply(as.matrix(count_data), 1, stats::mad)
    keep_mad_lnc <- unique(c(lnc_keep, names(sort(mads, decreasing=TRUE))[1:min(top_var, sum(mads>0))]))
    filtered_data <- count_data[rownames(count_data) %in% keep_mad_lnc, , drop=FALSE]
  ...
  count_data <- count_data[keep_genes, ]                                   # genes x samples
  ...
  if (filterMethod == "DEG") {
    ...
    filtered_expr <- select_by_variance(count_data, topVarGenes, lnc_keep)
  } else {
    stop("Invalid filterMethod. Choose 'DEG' or 'var'.")
  }

  y <- edgeR::DGEList(counts=filtered_expr)
  y <- edgeR::calcNormFactors(y, method="TMM")
  vw <- limma::voom(y, plot=FALSE)    # log2-CPM with sample weights
  transformed_data <- as.data.frame(vw$E)   

  # Update the lacenObject with the transformed expression data
  lacenObject$datExpr <- data.matrix(t(transformed_data), rownames.force = TRUE)
  return(lacenObject)
}